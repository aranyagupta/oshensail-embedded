...
const unsigned int Pow2[14] = {1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096};
const char RSSdata[20] = {7, 6, 6, 5, 5, 5, 4, 4, 4, 4, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2};
...
4. out_r    : Output maginutes
5: out_im   : Output frequencies
...
void Approx_FFT(char in[], uint16_t N, float Frequency, int32_t out_r[], int32_t out_im[])
{
  ...
  for (int i = 0; i < a; i++) // getting  min max and average for scalling
  {
    out_r[i] = 0;
    out_im[i] = 0;
    // Conversion from stored chars to ints as three bytes big endian comprise signed PCM24
    // need to keep last byte as 0x00, but shouldn't be an issue (find some use for it?)
    int32_t bytesToInt = (in[3*i]<<24)+(in[3*i+1]<<16)+(in[3*i+2]<<8);
    data_avg = data_avg + bytesToInt;
    if (bytesToInt > data_max)
    {
      data_max = bytesToInt;
    }
    if (bytesToInt < data_min)
    {
      data_min = bytesToInt;
    }
  }
  ...
  // Break out if data mag is 0 - implies some underlying error
  if (data_mag == 0){
    return;
  }
  ...
  if (data_mag > 1024)
  {
    for (int i = 0; i < a; i++)
    {
      int32_t bytesToInt = (in[3*i]<<24)+(in[3*i+1]<<16)+(in[3*i+2]<<8);
      bytesToInt -= data_avg;
      bytesToInt = bytesToInt >> scale;
      in[3*i] = (bytesToInt&0xFF000000) >> 24;
      in[3*i+1] = (bytesToInt&0x00FF0000) >> 16;
      in[3*i+2] = (bytesToInt&0x0000FF00) >> 8;
    }
    scale = 128 - scale;
  }
  ...
  {
    int32_t bytesToInt = (in[3*out_im[i]] << 24) + (in[3*out_im[i]+1] << 16) + (in[3*out_im[i]+2] << 8);
    out_r[i] = bytesToInt;
    out_im[i] = 0;
  }
...
{
  int temp3, m1, m2;
  char temp1, temp2, test, quad, accuracy;
